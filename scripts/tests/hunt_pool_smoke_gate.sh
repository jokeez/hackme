...
echo "[hunt-pool-gate] run workerfuzz"
for w in hunt-w1 hunt-w2; do
  WORKER_ID="$w" timeout 120s "$WORKERFUZZ_BIN" -coord "$BASE" -token "$HACKME_COORDINATOR_WORKER_TOKEN" -worker "$w" 2>/dev/null || true
done
