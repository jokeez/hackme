...
rem Repair env if pool token line is empty (common after manual edits / old zip)
set "POOL_OK=0"
for /f "usebackq delims=" %%T in ("pool.miner.token") do if not "%%T"=="" set "POOL_OK=1"
if "!POOL_OK!"=="1" (
  for /f "tokens=1,* delims==" %%A in ('findstr /B /I "HACKME_POOL_COORDINATOR_TOKEN=" hackme.env 2^>nul') do (
...
if "!HACKME_POOL_COORDINATOR_TOKEN!"=="" (
  for /f "usebackq delims=" %%T in ("pool.miner.token") do set "HACKME_POOL_COORDINATOR_TOKEN=%%T"
)
if "!HACKME_POOL_COORDINATOR_TOKEN!"=="" (
