#!/usr/bin/env python3
"""Export ckpool fuzz pack JSON → static fuzz_report_v2 HTML."""
...
GUARD_WASM = {
    "v1_line_frame": "tasks/artifacts/security/ckpool/v1_line_frame.wasm",
    "submit_hex_fields": "tasks/artifacts/security/ckpool/submit_hex_fields.c",
    "version_mask": "tasks/artifacts/security/ckpool/version_mask.wasm",
    "submit_param_count": "tasks/artifacts/security/ckpool/submit_param_count.wasm",
    "ntime_window": "tasks/artifacts/security/ckpool/ntime_window.wasm",
}
GUARD_WASM["submit_hex_fields"] = "tasks/artifacts/security/ckpool/submit_hex_fields.wasm"
...
def fix_repro(cmd: str, guard: str) -> str:
    if not cmd:
        return ""
    wasm = GUARD_WASM.get(guard, GUARD_WASM["v1_line_frame"])
...
<a href="https://github.com/mempool/ckpool">mempool/ckpool</a> (Con Kolivas) parser paths
(<code>connector.c</code> MAX_MSGSIZE 1024, <code>stratifier.c</code> parse_submit) — not native ASAN on ckpool binary.
...
def render_guard_page(report: dict, guard: str, gen: str, back: str = "../") -> str:
    ...
    cid = camp.get("id") or ""
    recs = [
        "0 critical in this pass — no urgent patch requested for ckpool.",
        "Re-run repro locally; validate any crash-class signal on native ckpool + ASAN before CVE claims.",
    ]
...
<p class="sub">fuzz_report_v2 · {esc(gen)} · <a href="https://hackme.tech" target="_blank" rel="noopener">hackme.tech</a> · <a href="{esc(back)}">ckpool pack</a></p>
{scope_block()}
<div class="card">
...
<footer>HackMe Network · fuzz_report_v2 · ckpool voluntary research · Generated {esc(gen)}</footer>
</div></body></html>"""
...
<html lang="en"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>HackMe Fuzz Report · ckpool parser pack</title>
<link rel="canonical" href="https://hackme.tech/reports/ckpool-fuzz/"/>
<style>{CSS}</style>
</head><body><div class="wrap">
<h1>HackMe Security Report</h1>
<p class="sub">fuzz_report_v2 · ckpool pack · {esc(gen)} · <a href="https://github.com/mempool/ckpool">mempool/ckpool</a></p>
{scope_block()}
<div class="card">
<p class="lbl">Campaign pack</p>
<p class="title">ckpool · Stratum parser boundary fuzz</p>
...
<p>No confirmed memory-safety bug from this WASM pass. Guard signals = malformed inputs ckpool should reject via <code>parse_client_msg</code> / <code>parse_submit</code>.</p>
</div>
...
def main() -> int:
    pack_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "reports/ckpool-fuzz"
    out_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "web/site/reports/ckpool-fuzz"
