...
lines += [
    ...
    f"",
    f"3-rig mining, settlement to HMC-91fe, phasing proven, dashboard stable.",
    f"",
    f"## What is NEXT (your planned order)",
    f"",
    ...
    f"3. **Phasing soak** — `phasing_soak_prep.sh`, `orders_matrix.sh`",
    f"4. **1M architecture** — see `docs/PRODUCTION_MASTER_ROADMAP.md` Phase 5",
    f"",
    f"Full roadmap: `docs/PRODUCTION_MASTER_ROADMAP.md`",
    f"",
    f"Artifacts: `{out}`",
]
verdict.write_text("\n".join(lines))
