#!/usr/bin/env bash
# rc11q: settlement API fix, miner notices banner, main.go split, lite status security flags.
#
#   bash scripts/ops/release_rc11q_publish.sh
#   SKIP_ISO=1 bash scripts/ops/release_rc11q_publish.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
VERSION="${VERSION:-0.1.0-rc11q}"
NODE_SSH="${NODE_SSH:-hackme-vps}"
SKIP_ISO="${SKIP_ISO:-0}"
SKIP_INSTALLER="${SKIP_INSTALLER:-0}"
SKIP_GATES="${SKIP_GATES:-0}"
NEWS_ID="${NEWS_ID:-2026-06-27-rc11q-settlement-banner}"

echo "[rc11q] version=$VERSION commit=$(git rev-parse --short=12 HEAD 2>/dev/null || echo nogit)"

if [[ "$SKIP_GATES" != "1" ]]; then
  echo "[rc11q] go test (short)"
...
echo "[rc11q] release bundle"
VERSION="$VERSION" bash scripts/release/make_release_bundle.sh

if [[ "$SKIP_INSTALLER" != "1" ]]; then
  echo "[rc11q] windows installer"
  VERSION="$VERSION" bash scripts/release/windows/build_installer.sh "$VERSION" || echo "[rc11q] WARN: installer build failed"
fi

if [[ "$SKIP_ISO" != "1" ]]; then
  echo "[rc11q] HackMe OS ISO"
...
echo "[rc11q] deploy hackme-node + coordinator"
...
FORCE_NEWS_ID="$NEWS_ID" NODE_SSH="$NODE_SSH" \
  bash scripts/ops/publish_news_to_telegram.sh || echo "[rc11q] WARN: telegram skipped"

echo "[rc11q] DONE — https://hackme.tech/downloads.html"
