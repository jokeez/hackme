#!/usr/bin/env bash
# Wave 9 — markdown/regex/compress/json: cmark, oniguruma, miniz, json-c (150k each).
#
#   bash scripts/ops/run_oss_cve_wave9.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT="${OUT:-$ROOT/reports/oss-cve/wave9-${STAMP}}"
TARGETS="${TARGETS:-cmark,oniguruma,miniz,json-c}"
BUDGET="${BUDGET:-150000}"
TIME_LIMIT="${TIME_LIMIT:-10800}"
SKIP_PACK_BUILD="${SKIP_PACK_BUILD:-1}"

mkdir -p "$OUT"
log() { echo "[wave9 $(date -u +%H:%M:%S)] $*" | tee -a "$OUT/wave9.log"; }

log "preflight build"
HACKME_REPO_ROOT="$ROOT" go test ./internal/fuzzupstream/... -count=1 \
  -run 'TestBuildAllTargets/(cmark|oniguruma|miniz|json-c)$' -timeout 25m >>"$OUT/build.log" 2>&1
