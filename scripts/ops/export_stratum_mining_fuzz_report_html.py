#!/usr/bin/env python3
"""Export stratum_mining fuzz pack JSON → static fuzz_report_v2 HTML."""
...
GUARD_WASM = {
    "sv2_frame_bounds": "tasks/artifacts/security/stratum_mining/sv2_frame_bounds.wasm",
    "extranonce_len": "tasks/artifacts/security/stratum_mining/extranonce_len.wasm",
    "user_identity_len": "tasks/artifacts/security/stratum_mining/user_identity_len.wasm",
    "open_channel_target": "tasks/artifacts/security/stratum_mining/open_channel_target.wasm",
}
...
def fix_repro(cmd: str, guard: str) -> str:
    if not cmd:
        return ""
    wasm = GUARD_WASM.get(guard, GUARD_WASM["sv2_frame_bounds"])
...
<a href="https://github.com/mempool/stratum_mining">mempool/stratum_mining</a> (Con Kolivas) parser paths
(<code>Sv2 frame bounds, extranonce/user-identity limits, open_channel target) — not native ASAN on stratum_mining binary.
<strong>Guard signals</strong> on malformed inputs are expected reject paths, not confirmed CVEs.</p></div>"""
...
def render_guard_page(report: dict, guard: str, gen: str, back: str = "../") -> str:
    ...
    cid = camp.get("id") or ""
    recs = [
        "0 critical in this pass — no urgent patch requested for stratum_mining.",
        "Re-run repro locally; validate any crash-class signal on native stratum_mining + ASAN before CVE claims.",
    ]
...
<p class="sub">fuzz_report_v2 · {esc(gen)} · <a href="https://hackme.tech" target="_blank" rel="noopener">hackme.tech</a> · <a href="{esc(back)}">stratum_mining pack</a></p>
{scope_block()}
<div class="card">
<p class="lbl">Campaign</p>
<p class="title">{esc(title)}</p>
<p class="muted"><code>{esc(cid)}</code> · property · status {esc(camp.get("status"))}</p>
<span class="badge" style="border-color:{vc};color:{vc}">{esc(ver)}</span>
<div class="grid">
...
<footer>HackMe Network · fuzz_report_v2 · stratum_mining voluntary research · Generated {esc(gen)}</footer>
</div></body></html>"""
...
def render_index(pack: dict, guards: list[dict], gen: str) -> str:
    ...
    nguards = len(guards) or 5
    return f"""<!DOCTYPE html>
<html lang="en"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>HackMe Fuzz Report · stratum_mining parser pack</title>
<link rel="canonical" href="https://hackme.tech/reports/stratum_mining-fuzz/"/>
<style>{CSS}</style>
</head><body><div class="wrap">
<h1>HackMe Security Report</h1>
<p class="sub">fuzz_report_v2 · stratum_mining pack · {esc(gen)} · <a href="https://github.com/mempool/stratum_mining">mempool/stratum_mining</a></p>
{scope_block()}
<div class="card">
<p class="lbl">Campaign pack</p>
<p class="title">stratum_mining · Stratum parser boundary fuzz</p>
<p class="muted">{nguards} WASM guards · property checks on connector + stratifier paths</p>
<span class="badge" style="border-color:{vc};color:{vc}">{total_crit} CRITICAL · {total_sig} GUARD SIGNALS</span>
<div class="grid">
<div class="stat"><b>Total runs</b>{total_runs}</div>
<div class="stat"><b>Guards</b>{nguards}</div>
<div class="stat"><b>Critical</b>{total_crit}</div>
<div class="stat"><b>High (sample)</b>{total_sig}</div>
</div></div>
<div class="card"><p class="lbl">Per-guard reports</p>
...
<p>No confirmed memory-safety bug from this WASM pass. Guard signals = malformed inputs stratum_mining should reject via <code>Sv2 wire decode / channel open</code>.</p>
</div>
...
def main() -> int:
    pack_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "reports/stratum_mining-fuzz"
    out_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "web/site/reports/stratum_mining-fuzz"
