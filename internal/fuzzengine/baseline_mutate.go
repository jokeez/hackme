...
// mutateBytesBaseline replicates upstream feature/hunt-mvp byte havoc (16 ops, shallow stack)
// for honest A/B calibration only — not used in production pool replay.
func mutateBytesBaseline(base []byte, stage MutationStage, salt uint64, maxLen int, dict []byte) []byte {
	if maxLen <= 0 {
