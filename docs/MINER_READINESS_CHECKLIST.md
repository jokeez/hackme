...
| ISO on CDN | SHA256 from `SHA256SUMS-iso.txt` on rc11s dist + GRUB “HackMe OS” (see below) | ☐ |
...
```bash
# After download:
sha256sum HackMe-OS-0.1.0-rc11s-amd64.iso
# Must match SHA256SUMS-iso.txt on https://hackme.tech/dist/release_0.1.0-rc11s/
